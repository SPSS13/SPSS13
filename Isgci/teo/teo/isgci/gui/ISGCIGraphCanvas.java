...
public class ISGCIGraphCanvas extends mxGraphComponent implements
	...
	private String vertexStyle = "shape=rectangle;fontColor=black;strokeColor=black;spacingLeft=4;spacingRight=4;spacingTop=2;spacingBottom=2;rounded=true";
	...
	public void drawNeighbours() {
		Set<GraphClass> selected = ((GraphClassSet) getSelectedCell()
				.getValue()).getSet();
	...
	/**
	 * Sets the name of the node to the name that was selected for
	 ...
	 *       Method of the ISGCIMainFrame. 
	 *       
	 *       Actually, centering only
	...
	/**
	 * Zooming Function by setting values after computing relative
	 *             to the mouse position for the Scrollbars
	 * 
	 * @date 21.06.2013
	 * @author Matthias
	 ...
	 *             when calling this method and the graphbounds are smaller than 3/4 of the windowsize, 
	 *             then will the graph zoomed to nearly completely fit in the window
	 *             if the graph is bigger then the window, respectively to windowsize zoomOut, so 
	 *             that the whole graph will be visible in the Window
	 * @version 28.06.2013
	...
	public void fitInWindow() {
		...
		try{
			...
			graphSize = ((mxGraphComponent) parent.getContentPane().getComponent(0))
					.getGraphControl().getSize();
