...
public class NodePopup extends JPopupMenu implements ActionListener {
	...
	public void show(Component orig, int x, int y) {
		...
		// TODO: Document
		miShowSuperclasses.setEnabled(superClassesMissing());
		miShowSubclasses.setEnabled(subClassesMissing());
		miHideSubclasses.setEnabled(parent.graphCanvas.getSubNodes(cell).size()>0&&!subClassesMissing());
		miHideSuperclasses.setEnabled(parent.graphCanvas.getSuperNodes(cell).size()>0&&!superClassesMissing());
//		miAddSubclasses.setEnabled(subClassesAvailable());
//		miAddSuperclasses.setEnabled(superClassesAvailable());
	...
	/**
	 * Method for calculating if there are more superclasses for the selected
	 * node in the drawn universe to show
	 * 
	 * @return true if there are more classes
	 * @author Fabian Vollmer
	 * @date 03.07.2013
	 */
	private boolean superClassesMissing() {
		List<Object> l = new ArrayList<Object>();
		if (parent.graphCanvas.getSuperNodes(cell).size() > 0) {
			for (GraphClass gc : parent.graphCanvas.getSuperNodes(cell)) {
				if(!((mxCell)parent.graphCanvas.findNode(gc)).isVisible()){
					l.add(parent.graphCanvas.findNode(gc));
				}
			}
		}
		System.out.println();
	...
	/**
	 * Method for calculating if there are more subclasses for the selected node
	 * in the drawn universe to show
	 * 
	 * @return true if there are more classes
	 * @author Fabian Vollmer
	 * @date 03.07.2013
	 */
	private boolean subClassesMissing() {
		List<Object> l = new ArrayList<Object>();
		if (parent.graphCanvas.getSubNodes(cell).size() > 0) {
			for (GraphClass gc : parent.graphCanvas.getSubNodes(cell)) {
				if(!((mxCell)parent.graphCanvas.findNode(gc)).isVisible()){
					l.add(parent.graphCanvas.findNode(gc));
				}
			}
	 ...
	 * @date 03.07.2013
	 */
	private boolean superClassesAvailable() {
		// TODO:
		return parent.graphCanvas.getSuperNodes(cell).size() > 0;
	}
	 ...
	 * @date 03.07.2013
	 */
	private boolean subClassesAvailable() {
		//TODO:
		List<GraphClass> l = new ArrayList<GraphClass>();
		...
		if (!DataSet.inclGraph.edgesOf(gc).isEmpty()) {
            for (Inclusion inc : DataSet.inclGraph.edgesOf(gc)) {
                l.add(inc.getSub());
                System.out.println(inc.getSub());
            }
        }		
		System.out.println(l.size());
		l.removeAll(((GraphClassSet)cell.getValue()).getSet());
		return l.size() > 0;
	}
}
